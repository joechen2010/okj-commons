...
public final class Globals {
     ...
     */
    private static final String DEFAULTS_PROPERTY_FILE = "application.properties";
    ...
    private Globals() {
		...
		try {
			customerGlobalProperties = loadCustomerGlobalProperties();
	 ...
	 */
	public static PropertiesConfiguration loadCustomerGlobalProperties() throws BaseException{

        PropertiesConfiguration customerGlobalProperties = null;
        String globalsPropertyFilePath = locateSysConfig();
        
        if (globalsPropertyFilePath == null){
			throw new BaseException("Customer Globals properties file path not specified.");
        }
        ...
        if ( (globalsPropertyFilePath.toLowerCase()).indexOf(".xml")>0 ) {
            ...
            try {
    			...
    			throw new BaseException("Could not load globals properties as XML from file at '"+globalsPropertyFilePath+"'.", e);
    		}
    	...
    	// parse legacy properties format
	        ...
	        try {
	            ...
	            throw new BaseException("Could not open globals properties file at '"+globalsPropertyFilePath+"'.", e);
	        }
     ...
     */
    private static String locateSysConfig() throws BaseException{
		...
		try {
			...
			throw new BaseException("Error loading global variables file.", e);
		}
